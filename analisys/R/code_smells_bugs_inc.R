...
file_server= paste0(path, "issues_bugs/cs_window_bugs/cs_metrics_window_" ,app , ".csv")
cs_metrics <- read.csv(file_server,header=TRUE,sep=";")
date_d <- as.Date(cs_metrics$Date)


file_issues=paste0(path, "issues_bugs/bugs_by_version/", app, "_bugs_by_version.csv")
issues <- read.csv(file_issues,header=TRUE,sep=";")
...
issues_df= cbind.data.frame(date = as.Date(issues$date2) , nissues = (issues$count))

z_issues = zoo(issues_df$nissues, issues_df$date)
x_issues = as.xts(z_issues)


issues0_df= cbind.data.frame(date = as.Date(issues$date1) , nissues = (issues$count))

z_issues0 = zoo(issues0_df$nissues, issues0_df$date)
x_issues0 = as.xts(z_issues0)
