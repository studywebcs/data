...
path2=paste0(path, "analysis/correlacao_4/rq5_time_to_release/data_granger/")

outfile1=paste0(path2, "lag1/granger_cs_ttr_", app, "_lag1.csv")
outfile2=paste0(path2, "lag2/granger_cs_ttr_", app, "_lag2.csv")
outfile3=paste0(path2, "lag3/granger_cs_ttr_", app, "_lag3.csv")
outfile4=paste0(path2, "lag4/granger_cs_ttr_", app, "_lag4.csv")
...
#i=6
    ...
    if (sum(cs_metrics[, i]) == 0){
      ...
      g1 = grangertest( x_versionage ~ x_series, order=1 )
      gp1=g1$`Pr(>F)`[2]
      if (gp1 <= 0.05 ){ gc1="*" } else { gc1="" }
      
      
      g2 = grangertest( x_versionage ~ x_series, order=2 )
      gp2=g2$`Pr(>F)`[2]
      ...
      if (unique <=3){
        ...
        g3 = grangertest( x_versionage ~ x_series, order=3 )
        gp3=g3$`Pr(>F)`[2]
        if (gp3 <= 0.05 ){ gc3="*" } else { gc3="" }

    
        g4 = grangertest( x_versionage ~ x_series, order=4 )
        gp4=g4$`Pr(>F)`[2]
